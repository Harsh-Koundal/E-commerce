...
export default function StampInks() {
  ...
  // Fetch stamp & ink categories from backend
  useEffect(() => {
    const fetchStampInk = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/accessorycategories/stamps-printing`
        );

        console.log("Fetched Stamp & Ink Category:", response.data);

        setSubCategories(response.data?.data?.subcategories || []);
      } catch (error) {
        console.error("Error fetching stamp & ink categories:", error);
      }
    };
  ...
  const handleOrderNow = (subcategoryId) => {
    navigate(`/order-page/stamps-printing?subcategory=${subcategoryId}`);
  };
    ...
    <div className="bg-gray-100 p-6 h-full">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="bg-[#FDF1F2] p-6 rounded-lg w-full md:w-2/3">
            ...
            <p className="text-gray-700 text-lg">
              ...
              <br />
              Custom stamps built for precision, durability, and everyday office workflow.
            </p>

            <ul className="list-disc list-inside mt-2">
              <li>Self-inking, pre-inked, and rubber stamps</li>
              <li>Custom logo, signature, seal & more</li>
              <li>Long-lasting inks in multiple colors</li>
              <li>Perfect for office, business & admin use</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="shadow-md p-5 bg-white rounded-lg mt-5">
        <h2 className="text-4xl font-bold text-gray-800 mb-8">
          Explore <span className="text-blue-600">Stamps & Inks</span> Subcategories
        </h2>

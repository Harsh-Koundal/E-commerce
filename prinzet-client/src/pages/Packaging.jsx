...
export default function LabelStickersPackaging() {
    ...
    const fetchCategories = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/accessorycategories/labels-printing`
        );

        setSubCategories(response.data?.data?.subcategories || []);
      } catch (error) {
        console.error("Error fetching Labels, Stickers & Packaging:", error);
      }
    };
  ...
  const handleOrderNow = (subcategoryId) => {
    navigate(`/order-page/labels-printing?subcategory=${subcategoryId}`);
  };
    ...
    <div className="bg-gray-100 p-6 h-full">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="bg-[#FDF1F2] p-6 rounded-lg w-full md:w-2/3">
            ...
            <p className="text-gray-700 text-lg">
              <b>Stick your story where it matters.</b>
              <br />
              From quirky stickers to polished packaging — strong branding starts here.
            </p>

            <ul className="list-disc ml-5 mt-2">
              <li>Die-cut, roll, or sheet format options</li>
              <li>Glossy, matte, waterproof finishes</li>
              <li>Perfect for small businesses & product branding</li>
              <li>Durable & vibrant print quality</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="shadow-md p-5 bg-white rounded-lg mt-5">
        <h2 className="text-4xl font-bold text-gray-800 mb-8">
          Explore <span className="text-blue-600">Labels, Stickers & Packaging</span> Subcategories
        </h2>
